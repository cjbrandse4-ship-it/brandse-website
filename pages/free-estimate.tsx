...
const copy = {
  en: {
    ...
    heroHeadline: 'Tree Services in West Island, Vaudreuil & Surrounding — Callback in 2 Hours',
    heroSub: 'Free written quote in 24h · Family-owned · Fully insured',
  ...
  fr: {
    ...
    heroHeadline: 'Services d\'arbres dans l\'Ouest de l\'Île, Vaudreuil et environs — Rappel en 2 heures',
    heroSub: 'Devis écrit gratuit en 24h · Entreprise familiale · Entièrement assuré',
          ...
          <div className="max-w-7xl mx-auto px-4 pt-10 pb-12 md:pt-16 md:pb-20 grid md:grid-cols-2 gap-8 md:gap-12 items-center">
            ...
            <div className="text-white">
              ...
              {/* Review chips — compact, brand-recognizable side-by-side pills
                  ...
                  hero carries the same info for mobile. */}
              <div className="mt-6 hidden md:flex flex-wrap items-center gap-2.5">
                <ReviewChip
                  logo={<GoogleGLogo className="w-4 h-4" />}
                  rating={isFr ? siteConfig.rating.value.replace('.', ',') : siteConfig.rating.value}
                  count={`${siteConfig.rating.count} ${c.reviewsWord}`}
                />
                <ReviewChip
                  logo={<FacebookLogo className="w-4 h-4" />}
                  rating={isFr ? '5,0' : '5.0'}
                  count={`14 ${c.reviewsWord}`}
                />
              </div>
            </div>
...
function ReviewChip({ logo, rating, count }: { logo: React.ReactNode; rating: string; count: string }) {
  return (
    <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/10 backdrop-blur-sm ring-1 ring-white/25 text-white">
      <span className="shrink-0">{logo}</span>
      <span className="text-sm font-semibold leading-none">{rating}</span>
      <Star className="w-3.5 h-3.5 fill-yellow-400 text-yellow-400 shrink-0" />
      <span className="text-xs text-white/80 leading-none">· {count}</span>
    </div>
  );
}
