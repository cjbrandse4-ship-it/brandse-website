...
const SaintLazarePage = () => {
  const { t, getLocalizedPath, language } = useLanguage();
  const isFr = language === 'fr';
  const city = "Saint-Lazare";
  ...
  const loc = {
    ...
    metaDesc: isFr ? `Services d'arbres professionnels à Saint-Lazare. Abattage, élagage et essouchage par des arboriculteurs certifiés. Urgence 24/7. Soumission gratuite.` : `Professional tree services in Saint-Lazare. Tree removal, trimming, and stump grinding by certified arborists. 24/7 emergency. Free quote.`,
    heroTitle: replaceCity(t('locationTemplate.heroTitle')),
    heroSubtitle: replaceCity(t('locationTemplate.heroSubtitle')),
  };
  const pageUrl = `https://servicedarbresbrandse.com${getLocalizedPath('/saint-lazare')}`;
  ...
  return (
    <>
      <SEOHead 
        ...
        canonicalUrl={getLocalizedPath('/saint-lazare')}
        ogUrl={getLocalizedPath('/saint-lazare')}
      />
      
      <LocalBusinessGeoSchema 
        city="Saint-Lazare" 
        address="Saint-Lazare, QC" 
        serviceArea="Saint-Lazare" 
        description={loc.metaDesc} 
        url={pageUrl} 
      />
      ...
      <BreadcrumbSchema breadcrumbs={[
        ...
        { label: city, url: '/services/saint-lazare' }
      ]} />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            <div className="lg:col-span-2 prose prose-lg text-gray-600">
              ...
              <p>
                {isFr 
                  ? "Saint-Lazare est caractérisée par ses vastes terrains boisés, son sol sablonneux unique et sa forte concentration de pins rouges et de conifères. L'entretien de ces grands arbres nécessite une expertise spécifique, particulièrement pour prévenir les maladies fongiques courantes dans les sols sablonneux." 
                  : "Saint-Lazare is characterized by its vast wooded lots, unique sandy soil, and high concentration of red pines and conifers. Maintaining these large trees requires specific expertise, particularly to prevent fungal diseases common in sandy soils."}
              </p>
              
              <h3 className="text-2xl font-bold text-gray-900 mt-8 mb-4">
                {isFr ? "Réglementations Locales" : "Local Regulations"}
              </h3>
              <p>
                {isFr 
                  ? "La ville de Saint-Lazare protège rigoureusement son couvert forestier. Un permis est requis pour l'abattage de tout arbre de plus de 10 cm de diamètre. De plus, des règles strictes s'appliquent pour le remplacement des arbres abattus afin de maintenir la densité forestière de la municipalité." 
                  : "The city of Saint-Lazare rigorously protects its forest canopy. A permit is required for the removal of any tree over 10 cm in diameter. Additionally, strict rules apply for replacing felled trees to maintain the municipality's forest density."}
              </p>
              <Link to={getLocalizedPath('/blog/how-to-plant-a-tree')} className="text-primary font-semibold hover:underline">
                {isFr ? "Guide sur la plantation d'arbres de remplacement" : "Guide on planting replacement trees"}
              </Link>
              ...
              <p>
                {isFr 
                  ? "Les grands pins de Saint-Lazare sont particulièrement vulnérables aux vents violents et aux tempêtes de verglas. L'élagage préventif et le haubanage sont des services essentiels que nous offrons pour sécuriser les propriétés équestres et résidentielles de la région." 
                  : "The large pines of Saint-Lazare are particularly vulnerable to high winds and ice storms. Preventive trimming and cabling are essential services we offer to secure equestrian and residential properties in the area."}
              </p>
            </div>
...
export default SaintLazarePage;
