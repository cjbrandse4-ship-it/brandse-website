...
const HudsonPage = () => {
  ...
  const loc = {
    ...
    metaDesc: isFr ? `Services d'arbres professionnels à Hudson. Abattage, élagage et essouchage par des arboriculteurs certifiés. Urgence 24/7. Soumission gratuite.` : `Professional tree services in Hudson. Tree removal, trimming, and stump grinding by certified arborists. 24/7 emergency. Free quote.`,
    heroTitle: replaceCity(t('locationTemplate.heroTitle')),
    heroSubtitle: replaceCity(t('locationTemplate.heroSubtitle')),
  };
  const pageUrl = `https://servicedarbresbrandse.com${getLocalizedPath('/hudson')}`;
  ...
  return (
    <>
      <SEOHead 
        ...
        canonicalUrl={getLocalizedPath('/hudson')}
        ogUrl={getLocalizedPath('/hudson')}
      />
      ...
      <BreadcrumbSchema breadcrumbs={[
        ...
        { label: city, url: '/services/hudson' }
      ]} />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            <div className="lg:col-span-2 prose prose-lg text-gray-600">
              ...
              <p>
                {isFr 
                  ? "Hudson est célèbre pour son charme rural, ses vastes propriétés boisées et son engagement envers la conservation de la nature. Les grands pins blancs, les pruches et les érables à sucre dominent le paysage. L'entretien de ces arbres majestueux nécessite une approche respectueuse de l'environnement." 
                  : "Hudson is famous for its rural charm, vast wooded properties, and commitment to nature conservation. Large white pines, hemlocks, and sugar maples dominate the landscape. Maintaining these majestic trees requires an environmentally respectful approach."}
              </p>
              
              <h3 className="text-2xl font-bold text-gray-900 mt-8 mb-4">
                {isFr ? "Conservation et Permis" : "Conservation & Permits"}
              </h3>
              <p>
                {isFr 
                  ? "La municipalité d'Hudson a des règlements de zonage stricts pour protéger son couvert forestier. L'abattage d'arbres est fortement réglementé, particulièrement près des cours d'eau et dans les zones de conservation. Nous vous aidons à naviguer dans ces règlements pour obtenir les permis nécessaires." 
                  : "The municipality of Hudson has strict zoning bylaws to protect its forest canopy. Tree removal is heavily regulated, especially near waterways and in conservation areas. We help you navigate these regulations to obtain the necessary permits."}
              </p>
              <Link to={getLocalizedPath('/blog/when-to-trim-trees')} className="text-primary font-semibold hover:underline">
                {isFr ? "Lisez notre guide sur le meilleur moment pour tailler" : "Read our guide on the best time to trim"}
              </Link>

              <h3 className="text-2xl font-bold text-gray-900 mt-8 mb-4">
                {isFr ? "Défis Climatiques Locaux" : "Local Climate Challenges"}
              </h3>
              <p>
                {isFr 
                  ? "Les propriétés d'Hudson, souvent exposées aux vents forts venant de la rivière des Outaouais, nécessitent un élagage préventif pour éviter les bris lors des tempêtes hivernales. Notre service d'urgence est souvent sollicité dans cette région après de forts coups de vent." 
                  : "Hudson properties, often exposed to strong winds from the Ottawa River, require preventive trimming to avoid breakage during winter storms. Our emergency service is frequently called to this area after heavy windstorms."}
              </p>
            </div>
